...
def test_generate_kv_cache_store_config_brackets_ipv6_master_service(tmp_path, monkeypatch):
    module = _load_mooncake_config_module()
    user_config = tmp_path / "user_config.json"
    output = tmp_path / "kv_store.json"
    user_config.write_text(json.dumps({"kv_cache_store_config": {"port": "50088"}}), encoding="utf-8")
    monkeypatch.setenv("KVS_MASTER_SERVICE", "2001:db8::1")

    assert module.generate_kv_cache_store_config(str(output), str(user_config)) is True
...
def test_generate_kv_conductor_config_brackets_ipv6_mooncake_endpoint(tmp_path, monkeypatch):
    ...
    user_config.write_text(
        json.dumps(
            {
                "kv_cache_store_config": {"port": "50088"},
